...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const lengthVector1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthVector2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  const dotProduct = x1 * x2 + y1 * y2;

  const angel = Math.acos(dotProduct / Math.abs(lengthVector1 * lengthVector2));
  return angel;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const summ = x1 + x2 + x3;
  return Number.isInteger(summ) ? summ : summ.toFixed(2);
}
...
function getCountOfOddNumbers(number) {
  return number !== 0 ? Math.ceil(Math.abs(number) / 2) : 0;
}
